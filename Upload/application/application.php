<?php
    ...
    class Web
    {
        public static function GetAccountBalance($user, $pass)
        {
            if (Auth::IsValidLogin($user, $pass))
            {
                ...
                $mysqli = Tool::NewWebConnection();
            
                if ($query = $mysqli->prepare('SELECT dp, vp FROM `account` WHERE `username` = ?'))
                {
                    $query->bind_param('s', $user);
                    $query->execute();
    ...
    class Tool
    {
        ...
        public static function NewWebConnection()
        {
            global $config;
            
            $conn = mysqli_connect
            (
                $config['mysql_web_hostname'],
                $config['mysql_web_user'],
                $config['mysql_web_pass'],
                $config['mysql_web_dbname'],
                $config['mysql_web_port']
            );
