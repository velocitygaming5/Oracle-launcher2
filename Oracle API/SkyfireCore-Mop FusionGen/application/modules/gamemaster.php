    ...
    public static function GetTicketsList($user, $pass)
    {
        if (Auth::IsValidLogin($user, $pass))
        {
            ...
            foreach ($config['mysqli']['realms'] as $realm)
            {
                ...
                {
                    if ($query = $mysqli->prepare('SELECT gm_tickets.ticketid, characters.name, characters.online, characters.race, characters.class, characters.gender, 
                        gm_tickets.message, gm_tickets.createTime, gm_tickets.lastModifiedTime, (SELECT NAME FROM characters WHERE guid = gm_tickets.assignedto)  AS AssignedTo,
                        gm_tickets.comment FROM gm_tickets LEFT JOIN characters ON gm_tickets.playerGuid = characters.guid WHERE gm_tickets.closedBy = 0 AND gm_tickets.completed = 0'))
                    {
                        $query->execute();
    ...
    public static function GetMuteLogs($user, $pass)
    {
        if (Auth::IsValidLogin($user, $pass))
        {
            ...
            if ($mysqli == true)
            {
                ...
                {
                    if ($query = $mysqli->prepare('SELECT account.username, account_muted.mute_date, account_muted.mute_date + (account_muted.mute_time*60) as unmuteDate, 
                        account_muted.mute_time, account_muted.muted_by, account_muted.mute_reason FROM account_muted LEFT JOIN account ON account.id = account_muted.id ORDER BY mutedate DESC'))
                    {
                        $query->execute();
                ...
                if (self::CanAccessPlayerInfo($gmAccountId, -2))
                {
                        ...
                        WHERE LOWER(characters.name) = LOWER(\''.$targetPlayerName.'\')'))
                    {
                        ...
                        $query->bind_result($accountId, $playerName, $playerRace, $playerClass, $playerGender, $playerLevel,
                            $guildName, $totalPlayedTime, $online, $lastLogout, $money, $totalKills, 
                        ...
                        while ($query->fetch()) 
                        {
                            ...
                            $row_array['arenaPoints'] = 0;
                            $row_array['totalHonorPoints'] = 0;
                            ...
                            if ($mysqliAuth == true)
                            {
                                if ($query3 = $mysqliAuth->prepare('SELECT 
                                    FROM_UNIXTIME(muteDate, \'%d %b %Y %h:%i:%s\') as mute_Date,
                                    CONCAT(mute_Time, \' minutes\') as muteTime, muted_By, mute_Reason
                            ...
                            $mysqliWeb = Web::NewDBConnection();
                            
                            if ($mysqliWeb == true)
                            {
                                if ($query4 = $mysqliWeb->prepare('SELECT dp, vp FROM `account_data` WHERE `id` = ?'))
                                {
