    ...
    public static function GetTicketsList($user, $pass)
    {
        if (Auth::IsValidLogin($user, $pass))
        {
            ...
            foreach ($config['mysqli']['realms'] as $realm)
            {
                ...
                {
                    if ($query = $mysqli->prepare('SELECT gm_complaint.id, characters.name, characters.online, characters.race, characters.class, characters.gender, 
                        gm_complaint.note, gm_complaint.createTime, gm_complaint.createTime, (SELECT NAME FROM characters WHERE guid = gm_complaint.assignedto)  AS AssignedTo,
                        gm_complaint.comment FROM gm_complaint LEFT JOIN characters ON gm_complaint.playerGuid = characters.guid WHERE gm_complaint.closedBy = 0'))
                    {
                        $query->execute();
            ...
            if ($mysqli == true)
            {
                if (self::CanAccessMuteLogs($accountId, -2))
                {
                    if ($query = $mysqli->prepare('SELECT account.username, account_muted.mutedate, account_muted.mutedate + (account_muted.mutetime*60) as unmuteDate, 
                        account_muted.mutetime, account_muted.mutedby, account_muted.mutereason FROM account_muted LEFT JOIN account ON account.id = account_muted.id ORDER BY mutedate DESC'))
                    {
                        $query->execute();
