...
class Auth
{
    ...
    public static function GetAccountRankName($user, $pass)
    {
        ...
        if (self::IsValidLogin($user, $pass))
        {
            ...
            if ($query = $mysqli->prepare('SELECT SecurityLevel FROM `account_access` WHERE AccountID = ?'))
            {
    ...
    public static function GetAccountRankId($accountId, $realmId = -2)
    {
        ...
        if ($mysqli == true)
        {
            $query = $mysqli->prepare('SELECT SecurityLevel, RealmID FROM account_access WHERE AccountID = ?');
