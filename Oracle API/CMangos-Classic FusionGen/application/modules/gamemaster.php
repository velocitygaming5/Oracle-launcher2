...
class GameMaster
{
            ...
            foreach ($config['mysqli']['realms'] as $realm)
            {
                ...
                if (self::CanReadTickets($accountId, $realm['id']))
                {
                    if ($query = $mysqli->prepare('SELECT 
                        ...
                        gm_tickets.created, 
                        gm_tickets.updated, 
                            (SELECT NAME FROM characters WHERE guid = gm_tickets.assignee_guid) AS AssignedTo,
                        gm_tickets.notes 
                        
                        FROM gm_tickets LEFT JOIN characters ON gm_tickets.author_guid = characters.guid WHERE gm_tickets.status = 0 AND gm_tickets.state = 0'))
                    {
                        $query->execute();
    ...
    public static function GetAllBansList($user, $pass)
    {
        ...
        // array_push($allBansArray, self::GetCharBansList($user, $pass)); // cmangos doesn't store character bans
        echo json_encode($allBansArray, JSON_PRETTY_PRINT);
    }
    
    // cmangos doesn't store mute logs
            ...
            if ($mysqliChar == true)
            {
                if (self::CanAccessPlayerInfo($gmAccountId, -2))
                {
                    if ($query = $mysqliChar->prepare('SELECT
                        ...
                        characters.money,
                        characters.stored_honor_rating,
                        characters.stored_honorable_kills,
                        FROM characters
                        ...
                        WHERE LOWER(characters.name) = LOWER(\''.$targetPlayerName.'\')'))
                    {
                        ...
                        $query->bind_result($accountId, $playerName, $playerRace, $playerClass, $playerGender, $playerLevel,
                            $guildName, $totalPlayedTime, $online, $lastLogout, $money, $totalHonorPoints, $totalKills);
                        ...
                        while ($query->fetch()) 
                        {
                            ...
                            $row_array['todayKills'] = 0;
                            $row_array['yesterdayKills'] = 0;
                            ...
                            $row_array['charBanLogs'] = array();
                            // $row_array['charBanLogs'] = self::GetCharacterBanLogsArray($targetPlayerName, $realmId); // cmangos doesn't store character bans
