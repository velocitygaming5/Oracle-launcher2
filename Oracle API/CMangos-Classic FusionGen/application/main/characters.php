...
class Characters
{
    ...
    public static function GetTopPvPList($limit)
    {
        ...
        foreach ($config['mysqli']['realms'] as $realm)
        {
            ...
            if ($query = $mysqli->prepare('SELECT `name`, `stored_honor_rating`, `stored_honorable_kills` 
                FROM `characters` WHERE `stored_honorable_kills` != 0 ORDER BY `stored_honorable_kills` DESC LIMIT ?'))
            {
                $query->bind_param('i', $limit);
                $query->execute();
                $query->bind_result($charName, $totalHonorPoints, $totalKills);
