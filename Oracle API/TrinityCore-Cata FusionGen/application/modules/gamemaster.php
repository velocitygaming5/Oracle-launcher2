    ...
    public static function GetTicketsList($user, $pass)
    {
        if (Auth::IsValidLogin($user, $pass))
        {
            ...
            foreach ($config['mysqli']['realms'] as $realm)
            {
                ...
                {
                    if ($query = $mysqli->prepare('SELECT gm_ticket.id, characters.name, characters.online, 
                        characters.race, characters.class, characters.gender, 
                        gm_ticket.description, gm_ticket.createTime, gm_ticket.lastModifiedTime,
                        (SELECT NAME FROM characters WHERE guid = gm_ticket.assignedto) AS AssignedTo,
                        gm_ticket.comment FROM gm_ticket LEFT JOIN characters ON gm_ticket.playerGuid = characters.guid 
                        WHERE gm_ticket.closedBy = 0 AND gm_ticket.completed = 0'))
                    {
                        $query->execute();
                ...
                if (self::CanAccessPlayerInfo($gmAccountId, -2))
                {
                        ...
                        WHERE LOWER(characters.name) = LOWER(\''.$targetPlayerName.'\')'))
                    {
                        ...
                        $query->bind_result($accountId, $playerName, $playerRace, $playerClass, $playerGender, $playerLevel,
                            $guildName, $totalPlayedTime, $online, $lastLogout, $money, $totalKills, $todayKills, $yesterdayKills);
