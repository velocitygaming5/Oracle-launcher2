...
class SinsHistory
{
    public static function GetSinsHistory($user, $pass)
    {
        if (Auth::IsValidLogin($user, $pass))
        {
            ...
            $row_array['charBanLogs'] = array(); // cmangos/mangos doesn't store character bans
    ...
    private static function GetAccountBansHistoryArr($accountName)
    {
        ...
        if ($mysqli == true)
        {
            if ($query = $mysqli->prepare('SELECT
                (SELECT `username` FROM account WHERE id = account_banned.account_id) AS accname,
                FROM_UNIXTIME(banned_at, \'%d %b %Y %h:%i:%s\') as banDate,
                FROM_UNIXTIME(unbanned_at, \'%d %b %Y %h:%i:%s\') as unbanDate,
                CONCAT(
                    FLOOR((unbanned_at-banned_at)/60/60/24), \'d \',
                    MOD(HOUR(SEC_TO_TIME(unbanned_at-banned_at)), 24), \'h \',
                    MINUTE(SEC_TO_TIME(unbanned_at-banned_at)), \'m \',
                    second(SEC_TO_TIME(unbanned_at-banned_at)), \'s\'
                ) as duration,
                account_banned.banned_by,
                account_banned.reason
                FROM account_banned
                LEFT JOIN account ON account_banned.account_id = account.id WHERE account_banned.account_id = ?
                ORDER BY account_banned.banned_at DESC'))
            {
