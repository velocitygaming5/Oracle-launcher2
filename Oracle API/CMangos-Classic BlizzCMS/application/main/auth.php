...
class Auth
{
    ...
    public static function GetAccountRankName($user, $pass)
    {
        ...
        if (self::IsValidLogin($user, $pass))
        {
            ...
            if ($query = $mysqli->prepare('SELECT gmlevel FROM `account` WHERE id = ?'))
            {
    ...
    public static function GetAccountRankId($accountId, $realmId = -2)
    {
        ...
        if ($mysqli == true)
        {
            $query = $mysqli->prepare('SELECT gmlevel FROM account WHERE id = ?');

            assert($query);

            $query->bind_param('i', $accountId);
            $query->execute();
            $query->bind_result($rankId);
    ...
    public static function IsValidLogin($user, $pass)
    {
        ...
        if ($mysqli == true)
        {
            if ($config['srp6'])
            {
                $query = $mysqli->prepare('SELECT username, s, v FROM account WHERE username = ?');

                assert($query);

                $query->bind_param('s',$user);
                $query->execute();
                $query->bind_result($dbUsername, $s, $v);

                while ($query->fetch())
                {
                    $client = new UserClient($user, $s);
                    $verifier = strtoupper($client->generateVerifier($pass));
    ...
    public static function GetAccountStanding($user, $pass)
    {
        ...
        if (self::IsValidLogin($user, $pass))
        {
            ...
            if ($query = $mysqli->prepare('SELECT banned_at, unbanned_at, UNIX_TIMESTAMP() as currTime FROM `account_banned` WHERE `active` = 1 AND account_id = ?'))
            {
                $query->bind_param('i', $accountId);
                $query->execute();
