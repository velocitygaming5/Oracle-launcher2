...
class CiSessions
{
    ...
    public static function LogUserGeo($user)
    {
        $sip = $_SERVER['REMOTE_ADDR'];
        $json = file_get_contents("http://ip-api.com/json/".$sip);
        ...
        if (!empty($data))
        {
            ...
            if ($mysqli == true)
            {
                if ($query = $mysqli->prepare('REPLACE INTO `ci_sessions_geo` (`user`, `city`, `country`, `isp`, `lat`, `lon`, `ip`, `regionName`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'))
                {
                    $query->bind_param('ssssssss', $user, $data->city, $data->country, $data->isp, $data->lat, $data->lon, $sip, $data->regionName);
                    $query->execute();

                    if ($mysqli->affected_rows != 0)
                    {
                        echo "User geo logged successfully. ";
                    }
                    else
                    {
                        echo "User geo wasn't recorded.";
                    }
                    $query->close();
                }
                mysqli_close($mysqli);
