        ...
        private async void BtnResetSearch_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(SearchBox.Text) && !string.IsNullOrWhiteSpace(SearchBox.Text))
                {
                    ...
                    LoadingOverlayPlaceholder.Content = $"Searching for player \"{SearchBox.Text}\" on realm: {cbSelectedRealm}";

                    pGmPanel.ShowActionMessage($"Searching for player {SearchBox.Text.ToUpper()} on realm: {cbSelectedRealm}");
                    ...
                    var label1 = new Label()
                    {
                        ...
                        FontSize = 14,
                        Content = "Please wait while retrieving player info.."
                    };
                    ...
                    var pInfoCollection = GameMasterClass.PlayerInfo.FromJson(await GameMasterClass.GetPlayerInfoJson(
                        OracleLauncher.LoginUsername, OracleLauncher.LoginPassword, SearchBox.Text, ((ComboBoxItem)CBRealms.SelectedItem).Tag.ToString()));
                    ...
                    if (SPPlayerInfo.Children.Count == 0)
                    {
                        var label2 = new Label()
                        {
                            ...
                            FontSize = 14,
                            Content = $"Player [{SearchBox.Text}] not found on realm: {cbSelectedRealm}!"
                        };
