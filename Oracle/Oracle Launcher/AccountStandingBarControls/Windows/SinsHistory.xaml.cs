using Oracle_Launcher.AccountStandingBarControls.Childs;
...
namespace Oracle_Launcher.AccountStandingBarControls.Windows
{
    /// <summary>
        ...
        public async void LoadSinsHistory()
        {
            try
            {
                var sinsHistoryCollection = SinsHistoryClass.SinsHistoryList.FromJson(
                    await SinsHistoryClass.GetSinsHistoryListJson(OracleLauncher.LoginUsername, OracleLauncher.LoginPassword));
            ...
            {
                ExceptionHandler.AskToReport(ex, "SinsHistory.xaml.cs", "LoadSinsHistory");
            }
            finally
            {
